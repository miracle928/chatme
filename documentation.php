<?php

include 'connect_db.php';

?>

<!DOCTYPE html>
<html lang="en">
    <head>
        <title>Chat me now! | Meet friends </title>
        <link rel="shortcut icon" type="icon" href="data:image/jpeg;base64,">
        <meta name="description" content="Are you bored? need someone to talk?, chatmenow is a social network designed to connect people around you">
        <meta name="keywords" content="Web chat, Social Network, Online Sharing">
        <meta name="author" content="Allen Charls Casul">
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
    </head>
    <body>
    <div class="wrapper">
		<article class="content">
        <h2>How does it work?</h2>
		    <p>The domain name chatmenow web app, was written in different languages and web technologies to ensure and stabilize the functionality of the website.</p><p><strong>These are the web languages used:</strong> Html5, Css3, Javascript, Ajax, PHP and MySQL database</p><p><strong>The owner doesn't collect your information for private purposes rather than monitoring unwanted user behavior such as exploiting pornographic content</strong></p>
            <h2>Services</h2>
            <p><strong>The owner of this website believed that digital communication also helps in socializing.</strong></p>
            <p>The site helps user to connect people around him/her.</p><p>It also allows to communicate other user through making convsersation</p><br><h2>Terms of use policy</h2>
			<p>This wesbite application (Web App) is an open source platform designed to bridge the connection between to or more random people anywhere from the globe.</p><p>To use without restriction and being reported, the user must agree and follow protocols implemented by the said owner</p><p><strong>These are the protocols you must follow:</strong><strong><p>1. Do not send any form of sexual information</p></strong><strong><p>2. Don't include pornography</p></strong><strong><p>3. Sexual exploitation is prohibited</p></strong><strong><p>4. Don't blackmail</p></strong><strong><p>5. Don't share private or sensitive information such as emails, password, address, and online banking information</p></strong><strong><p>6. You can use Virtual Private Network or VPN to protect or to hide your IP address (If needed)</p></strong><br>
        </article>
        <center><a class="proceed" href="index.php">Home</a></center><br><br>
        <style type="text/css">
            body {
    margin: 0;
    padding: 0;
}

.wrapper {
    width: 100%;
}

.wrapper p {
    font-family: arial;
    font-size: 19px;
    color: #111b;
    padding: 0px 30px 0px;
}

.wrapper h2 {
    color: gray;
    font-family: arial;
    padding: 30px 30px 0px;
}

.content {
    width: 100%;
}

.img {
    width: 100%;
}

.jpg {
    width: 300px;
    padding: 0px 30px 30px;
}

@media only screen and (min-width: 360px){
    .jpg {
    width: 100px;
    padding: 0px 125px 0px;
    margin-top: 20px;
}
.wrapper h2 {
    color: gray;
    font-family: arial;
    padding: 0px 30px 0px;
}

.content {
    width: 100%;
}

}

@media only screen and (min-width: 414px){
    .jpg {
    width: 85%;
    padding: 0px 29px 0px;
   }
}

@media only screen and (min-width: 768px){
    .wrapper h2 {
    color: gray;
    font-family: arial;
    padding: 30px 60px 0px;
    font-size: 50px;
   }

   .wrapper p {
    font-family: arial;
    font-size: 19px;
    color: #111b;
    padding: 0px 60px 10px;
    font-size: 25px;
   }

   .jpg {
    width: 85%;
    padding: 0px 60px 0px;
   }


}


@media only screen and (min-width: 1280px) {
    .jpg {
    width: 40%;
    padding: 0px 60px 0px;
   }
    .wrapper p {
    font-family: arial;
    font-size: 19px;
    color: #111b;
    padding: 0px 60px 0px;
    font-size: 25px;
   }
}


.form-section {
    margin: 0;
    padding: 0;
}

.input {
    border: none;
    width: 220px;
    height: 30px;
    font-size: 14px;
    font-family: arial;
    color: gray;
    padding: 0px 34px 5px;
    outline: none;


}

hr {
    width: 290px;
    margin-top: 0px !important;
}

.btn {
    width: 292px;
    padding: 12px;
    margin-top: 5px;
    margin-bottom: 50px;
    border: none;
    outline: none;
    color: white;
    background-color: gray;
    font-family: arial;
    cursor: pointer;
}


@media only screen and (min-width: 360px){
   .form-section {
    margin: 0;
    padding: 0;
}

.input {
    border: none;
    width: 220px;
    height: 30px;
    font-size: 14px;
    font-family: arial;
    color: gray;
    padding: 0px 34px 5px;
    outline: none;


}

hr {
    width: 290px;
    margin-top: 0px !important;
}

.proceed {
    width: 292px;
    padding: 12px 30px;
    margin-bottom: 50px;
    border: none;
    outline: none;
    color: white;
    background-color: gray;
    font-family: arial;
    cursor: pointer;
    text-decoration: none;
    line-height: 60px;
    border-radius: 50px;
    font-size: 14px;
}

.proceed:hover {
    background-color: rgb(61, 61, 61);
    color: rgb(252, 252, 252);
    transition: .5s;
}

}

@media only screen and (min-width: 414px){
    .input {
    border: none;
    width: 220px;
    height: 30px;
    font-size: 14px;
    font-family: arial;
    color: gray;
    padding: 0px 63px 5px;
    outline: none;
}
}

@media only screen and (min-width: 768px){
    .input {
    border: none;
    width: 220px;
    height: 30px;
    font-size: 14px;
    font-family: arial;
    color: gray;
    padding: 0px 250px 5px;
    outline: none;
}
}


@media only screen and (min-width: 1280px){
    .input {
    border: none;
    width: 220px;
    height: 30px;
    font-size: 14px;
    font-family: arial;
    color: gray;
    padding: 0px 490px 5px;
    outline: none;
}
}

@media only screen and (min-width: 1366px){
    .input {
    border: none;
    width: 220px;
    height: 30px;
    font-size: 14px;
    font-family: arial;
    color: gray;
    padding: 0px 39.3% 5px;
    outline: none;
}



}
        </style>
    </body>
    </html>